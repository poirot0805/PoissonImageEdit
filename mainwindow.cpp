...
void MainWindow::CreateActions()
{
    ...
    // File IO
    action_open_ = new QAction(QIcon(":/image/open.jpg"), tr("&Open..."), this);
    action_open_->setShortcuts(QKeySequence::Open);
    action_open_->setStatusTip(tr("Open an existing file"));
    connect(action_open_, SIGNAL(triggered()), this, SLOT(Open()));

    action_save_ = new QAction(QIcon(":/image/save.jpg"), tr("&Save"), this);
    ...
    action_import=new QAction(tr("Import Grad"), this);
    connect(action_import, SIGNAL(triggered()), this, SLOT(SetModeImport()));
    action_mixed=new QAction(tr("Mixed Grad"), this);
    connect(action_mixed, SIGNAL(triggered()), this, SLOT(SetModeMixed()));
}
...
void MainWindow::CreateToolBars()
{
    ...
    toolbar_file_->addAction(action_import);
    toolbar_file_->addAction(action_mixed);
...
void MainWindow::SetModeImport(){
    ChildWindow* window = GetChildWindow();
    if (!window)
        return;
    window->imagewidget_->set_draw_status_to_import();
    window->imagewidget_->set_source_window(child_source_);
}
void MainWindow::SetModeMixed(){
    ChildWindow* window = GetChildWindow();
    if (!window)
        return;
    window->imagewidget_->set_draw_status_to_mixed();
    window->imagewidget_->set_source_window(child_source_);
}
